...
echo ""
echo "Flash:  sudo dd if=debian-zero3e.img of=/dev/sdX bs=4M status=progress conv=fsync"
echo "Resize: sudo parted /dev/sdX resizepart 1 100% && sudo resize2fs /dev/sdX1"
echo "Serial: picocom -b 1500000 /dev/ttyUSB0"
echo "Root password: radxa  (CHANGE ON FIRST BOOT: passwd)"